...
import com.ckview.utils.db.Orm;
...
public abstract class AbstractDAO<T> {
	...
	public OpResult insert(T bean){
		...
		Orm orm = ParseTableXML.tables.get(getTableName());
		// Transform object to ContentValues
		ContentValues contentValues = BeanUtils.transformBeanToContentValues(getBeanClass(), bean, orm.getNameMap());
		//判断xml配置文件中主键是否有自增属性，如果有则移除该属性
		if(orm.getKey().isIdentity()){
			contentValues.remove(orm.getKey().getName());
		}
	...
	public OpResult update(T bean, String whereClause, String[] whereArgs){
		...
		Orm orm = ParseTableXML.tables.get(getTableName());
		ContentValues values = BeanUtils.transformBeanToContentValues(getBeanClass(), bean, orm.getNameMap());
		try {
			int count = db.update(getTableName(), values, whereClause, whereArgs);
			result = OpResult.success();
			result.setData(count);
	...
	public List<T> select(String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit){
		...
		while(cursor.moveToNext()){
			result.add((T) BeanUtils.getBean(getBeanClass(), cursor, orm.getNameMap()));
		}
		cursor.close();
		return result;
	}
